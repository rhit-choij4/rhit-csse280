...
rhit.DetailPageController = class {
	...
	updateView() {
		...
		fetch(`https://api.funtranslations.com/translate/pirate.json?text= ${rhit.fbSingleQuoteManager.quote}`, {
			method: 'GET',
			headers: {
				'X-Funtranslations-Api-Secret': 'fQ2JMSbx8Iq7MJrYpWZkkAeF'
			}
		}).then(response => response.json())
		.then(data => {
		console.log("Pirate quote: ", data.contents["translated"]);
		const posterImgEl = document.querySelector("#cardPoster");
		const quote = data.contents["translated"];
		quote.replace(/([\.,])/g, "$&\n");
        posterImgEl.src = `https://cataas.com/cat/says/${encodeURIComponent(quote)}`;
        posterImgEl.alt = rhit.fbSingleQuoteManager.movie;
		})
