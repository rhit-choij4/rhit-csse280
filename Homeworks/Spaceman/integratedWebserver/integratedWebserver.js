...
app.get("/api/player/guess/:id/:letter", function(req, res) {
    ...
    let instances = [];
    for(let i = 0; i < data[req.params.id].length; i++) {
        if(data[req.params.id].toLowerCase()[i] == req.params.letter){
            instances.push(i);
        }
    }

    res.send({letter: req.params.letter, length: data[req.params.id].length, index: req.params.id, locations: instances})
